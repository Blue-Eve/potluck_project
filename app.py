...
from potluck_code.model import recipe_search
import pandas as pd
...
with st.container():
    ...
    with right_column:
        st.write(
            "Created by Tim, Eva, Omar and Alex, Pot Luck is here to help make the most of what is in your cupboards."
        )
        st.write("Not had time to go shopping yet this week?")
        st.write(
            "Do you have a numerous random ingredients leftover that you are not sure what to do with?"
        )
    ...
    with left_column:
        with st.form("Where the magic happens..."):
            ...
            ingredients = st.text_area(
                label="**Enter your ingredients here, separated by a comma**"
            )
            ...
            if submission and ingredients == "":
                ...
                prediction = recipe_search(model, df, ingredients.split(","), int(mix))
        ...
        for i in range(recipes):
            ...
            with col:
                ...
                with left:
                    ...
                    st.markdown(
                        f'<h5 style="color:#f4976c">{name}</h5>', unsafe_allow_html=True
                    )
                with right:
                    rating = f"Average Rating {prediction.iloc[i]['avg_rating']}:star:"
                    st.markdown(
                        f'<p1 style="color:#f4976c">{rating}</p1>',
